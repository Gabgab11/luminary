...
function initGate(){
  ...
  form.addEventListener("submit", (event) => {
    ...
    // Personalize every spot on the page that shows the visitor's name
    document.querySelectorAll(".js-first-name").forEach((el) => {
      el.textContent = firstName;
    });
    const visitorChip = document.querySelector(".js-visitor-name");
    if (visitorChip) visitorChip.textContent = `${firstName} ${lastName}`;

    // Dismiss the gate and unlock the page underneath
    gate.classList.add("is-hidden");
    document.body.classList.remove("gate-active");

    // Optional: forward the lead to your own CRM/inbox here, e.g.
    // fetch("https://your-endpoint.example.com/leads", { method: "POST", body: JSON.stringify({ firstName, lastName, email }) });
  });
}
